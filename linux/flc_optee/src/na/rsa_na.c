...
static void prepare_ta_session(struct ta_attrs *ta)
{
...
static void terminate_tee_session(struct ta_attrs *ta)
{
	TEEC_CloseSession(&ta->sess);
	TEEC_FinalizeContext(&ta->ctx);
}

static void prepare_op(TEEC_Operation *op, char *in, size_t in_sz, char *out, size_t out_sz) {
...
static void rsa_gen_keys(struct ta_attrs *ta) {
	TEEC_Result res;
...
static void rsa_encrypt(struct ta_attrs *ta, char *in, size_t in_sz, char *out, size_t out_sz)
{
	TEEC_Operation op;
	uint32_t origin;
	TEEC_Result res;
...
static void rsa_decrypt(struct ta_attrs *ta, char *in, size_t in_sz, char *out, size_t out_sz)
{
	TEEC_Operation op;
	uint32_t origin;
	TEEC_Result res;
